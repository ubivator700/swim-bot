...
def adult_day(user_data):
    ...
    worker.send_msg(user_data['user_id'], keyboard=kb,botmessage='Какие дни и время для тебя удобны, '+user_data['user_name']+'? Отправь мне в сообщении имя тренера, группы которого тебе подходят. Я познакомлю тебя с ним 🤝🏻')
    db.set_position(user_data['user_id'], 'adult-trainer')
